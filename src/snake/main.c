...
static int is_hit_wall(void) {
  return ((snake.head->row <= 0) || (snake.head->col <= 0)) ||
         (snake.head->row >= row_max - 1) || (snake.head->col >= col_max - 1);
}
 ...
 * 清空整个地图
 */
void clear_map(void) { printf("%s", ESC_CLEAR_SCREEN); }
...
static void create_food(void) {
  ...
  do {
    ...
    // 食物不能在墙上
    if ((food->row < 0) || (food->row >= row_max) || (food->col < 0) ||
        (food->col >= col_max)) {
      continue;
    }
...
int main(int argc, char **argv) {
  ...
  do {
    ...
    if ((snake.status == SNAKE_BIT_ITSELF) ||
        (snake.status == SNAKE_BIT_WALL)) {
