...
int fatfs_mount(struct _fs_t *fs, int major, int minor) {
  ...
  // 分配一页来作为dbr区域的缓冲区
  dbr_t *dbr = (dbr_t *)memory_alloc_page(1);
  if (!dbr) {
  ...
  fat->data_start_sector =
      fat->root_start_sector + fat->root_ent_cnt * 32 / dbr->BPB_BytsPerSec;
  ...
  if (kernel_strncmp((char *)(dbr->BS_FilSysType), "FAT16", 5) !=
      0) {  // 不是FAT16文件系统，出错
...
int fatfs_read(char *buf, int size, file_t *file) {
  ...
  while (nbytes > 0) {
    ...
    uint32_t start_sector =
        fat->data_start_sector + (file->cblk - 2) * fat->sec_per_cluster;
