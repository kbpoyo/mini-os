...
static int cluster_is_valid(cluster_t cluster) {
  return (cluster < FAT_CLUSTER_INVALID) && (cluster >= FAT_CLUSTER_DAT_START);
}
...
static cluster_t cluster_alloc_free(fat_t *fat, int cnt) {
  ...
  // 0,1簇号未使用
  for (curr = FAT_CLUSTER_DAT_START; cnt && (curr < c_total); ++curr) {
  ...
  // 设置最后一个簇的后继为簇结束标志
  if (cnt == 0) {
    int err = cluster_set_next(fat, pre, FAT_CLUSTER_END);
    if (err < 0) {
      cluster_free_chain(fat, start);
      return FAT_CLUSTER_INVALID;
    }
...
static int expand_file(file_t *file, int inc_bytes) {
  ...
  if (file->size % fat->cluster_bytes_size == 0) {
    ...
    // 以簇为单位，计算需要拓展的簇的数量
    cluster_cnt =
        up2(inc_bytes, fat->cluster_bytes_size) / fat->cluster_bytes_size;
  } else {
    // 文件原始分配的最后一个簇空间还未装满
    // 计算最后一个簇的空间余量
    int cluster_free =
        fat->cluster_bytes_size - (file->size % fat->cluster_bytes_size);
    ...
    // 余量不足以支持拓展字节量，需要分配新簇
    cluster_cnt = up2(inc_bytes - cluster_free, fat->cluster_bytes_size) /
                  fat->cluster_bytes_size;
  }
...
static int move_file_pos(file_t *file, fat_t *fat, uint32_t move_bytes,
  ...
  if (c_offset + move_bytes >= fat->cluster_bytes_size) {
    ...
    if (next == FAT_CLUSTER_END && expand) {
...
file_type_t diritem_get_type(diritem_t *diritem) {
  ...
  if (diritem->DIR_Attr &
      (DIRITEM_ATTR_VOLUME_ID | DIRITEM_ATTR_SYSTEM |
 ...
 * @param p_index
 */
static void read_from_diritem(fat_t *fat, file_t *file, diritem_t *item,
                              int index) {
...
static int write_dir_entry(fat_t *fat, diritem_t *item, int dir_index) {
  ...
  // 将该目录项拷贝到扇区缓存的指定对应位置
  kernel_memcpy(fat->fat_buffer + offset % fat->bytes_per_sector, item,
                sizeof(diritem_t));
...
int fatfs_mount(struct _fs_t *fs, int major, int minor) {
  ...
  // 分配一页来作为dbr区域的缓冲区
  dbr32_t *dbr = (dbr32_t *)memory_alloc_page(1);
  if (!dbr) {
  ...
  fat->tbl_sectors = dbr->BPB_FATSz32;
  fat->tbl_cnt = dbr->BPB_NumFATs;
  fat->sec_per_cluster = dbr->BPB_SecPerClus;
  fat->cluster_bytes_size = dbr->BPB_SecPerClus * dbr->BPB_BytsPerSec;
  // fat32中2号簇默认分配给根目录区使用
  fat->root_ent_cnt = fat->cluster_bytes_size / sizeof(diritem_t);
  fat->root_start_sector =
      fat->tbl_start_sector + fat->tbl_sectors * fat->tbl_cnt;
  // 数据起始扇区就是根目录区，只是第一个簇(2号)默认给根目录区使用
  fat->data_start_sector = fat->root_start_sector;
  ...
  if (kernel_strncmp((char *)(dbr->BS_FilSysType), "FAT32", 5) !=
      0) {  // 不是FAT32文件系统，出错
  ...
  fat->fat_buffer = memory_alloc_page(
      up2(fat->cluster_bytes_size, MEM_PAGE_SIZE) / MEM_PAGE_SIZE);
...
mount_failed:
  ...
  if (fat->fat_buffer) {
    memory_free_page(
        fat->fat_buffer,
        up2(fat->cluster_bytes_size, MEM_PAGE_SIZE) /
            MEM_PAGE_SIZE);
  }
...
int fatfs_open(struct _fs_t *fs, const char *path, file_t *file) {
  ...
  if (file_item) {  // 从目录项中读取文件信息到file结构中
    ...
    if (file->mode & O_TRUNC) {  // 以截断模式打开文件，需清空文件
      ...
      file->cblk = file->sblk = FAT_CLUSTER_END;
      file->size = 0;
    }
    return 0;
  } else if ((file->mode & O_CREAT) &&
             p_index >=
                 0) {  // 创建文件模式下未找到对应的目录项，创建新一个文件
    // 初始化一个目录项信息
    diritem_t item;
...
int fatfs_read(char *buf, int size, file_t *file) {
  ...
  while (nbytes > 0) {
    ...
    uint32_t start_sector =
        fat->data_start_sector + (file->cblk - FAT_CLUSTER_DAT_START) * fat->sec_per_cluster;

    // 当前读取位置刚好在簇的开头，且读取大小为一个簇，直接进行整簇读取即可
    if (cluster_offset == 0 && nbytes == fat->cluster_bytes_size) {
      int err =
          dev_read(fat->fs->dev_id, start_sector, buf, fat->sec_per_cluster);
      if (err < 0) {
        return total_read;
      }
      ...
      int err = dev_read(fat->fs->dev_id, start_sector, fat->fat_buffer,
                         fat->sec_per_cluster);
      if (err < 0) {
        return total_read;
      }
...
int fatfs_write(char *buf, int size, file_t *file) {
  ...
  while (nbytes > 0) {
    ...
    if (cluster_offset == 0 && nbytes == fat->cluster_bytes_size) {
      int err =
          dev_write(fat->fs->dev_id, start_sector, buf, fat->sec_per_cluster);
      if (err < 0) {
        return total_write;
      }
      ...
      // 再将fat_buffer覆盖回磁盘
      err = dev_write(fat->fs->dev_id, start_sector, fat->fat_buffer,
                      fat->sec_per_cluster);
      if (err < 0) {
        return total_write;
      }
...
int fatfs_stat(file_t *file, struct stat *st) { return 0; }
 ...
 * @return int
 */
int fatfs_closedir(struct _fs_t *fs, DIR *dir) { return 0; }
