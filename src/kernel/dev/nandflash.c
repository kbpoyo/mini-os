...
int nand_read_page(uint32_t page_number, char* buf) {
  ...
  mecc0 =
      NF_RDDATA();  // 读spare区的前4个地址内容，即第2048~2051地址，这4个字节为main区的ECC
  ...
  secc =
      NF_RDDATA();  // 继续读spare区的4个地址内容，即第2052~2055地址，其中前2个字节为spare区的ECC值
...
uint8_t nand_write_page(uint32_t page_number, const char* buf) {
  ...
  if (temp == MARK_BAD_BLOCK) return -1;  // 是坏块，返回-1
