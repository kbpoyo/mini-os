...
void swi_handler(syscall_frame_t* frame) {
    if (frame->syscall_args->id < sizeof(sys_table) / sizeof(sys_table[0])) {    //当前系统调用存在
        ...
        if (handler) {
            ...
            int ret = handler(frame->syscall_args->arg0, frame->syscall_args->arg1, frame->syscall_args->arg2, frame->syscall_args->arg3);
